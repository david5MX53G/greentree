...
public class AuthController implements ActionListener {
	/** provides the elements feeding events to this <code>{@link ActionListener}</code> */
	 ...
	 * 
	 * @param jFrame {@link AuthJFrame} object that will receive handlers from this
	 * @param main {@link MainJFrameController} that stores the key and ciphertext of the session
	 * @param manager {@link GreenTreeManager} object that stores the {@link Token} of the user and
	 *   manages service layer actions
	 */
	...
	/**
	 * Invokes one of the <code>{@link AuthController}</code> methods based on the <code>
	 * {@link AuthJFrame}</code> element sending the <code>{@link ActionEvent}</code>
	 * 
	 * @param e ActionEvent used to determine which <code>TokenPrompt</code> element sent the <code>ActionEvent</code>
	 */
	@Override
	 ...
	 * com.greentree.view.AuthJFrame#getPassword()}</code>, then submits these to <code>{@link 
	...
	private void submitBtn_actionPerformed(ActionEvent ev) {
		String msg = null;
		System.out.println(logPrefix + ": submitBtn_actionPerformed(ActionEvent)");
		...
		try {
			...
			if (mainController.getCiphertext() != null) {
				if (manager.registerToken(mainController.getKey(), mainController.getCiphertext())) {
					...
					System.out.println(logPrefix+ ": manager.registerToken(this.key, ciphertext) FAILED");
			...
			System.out.println(logPrefix + ex.getClass().getName() + ": " + ex.getMessage());
	...
	private void keyBtn_actionPerformed(ActionEvent ev) {
		System.out.println(logPrefix + ": " + ev.getActionCommand());
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
    		...
    		try (ObjectInputStream input = new ObjectInputStream(new FileInputStream(file))) {
                System.out.println(logPrefix + ": opened " + file.getName() + ".");
            ...
            System.out.println(logPrefix + ": JFileChooser not approved");
        }
	}
}
