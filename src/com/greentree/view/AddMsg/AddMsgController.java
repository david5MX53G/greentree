...
public class AddMsgController implements ActionListener {
	...
	/**
	 * @param jFrame {@link AddMsgJFrame} sends events to this {@link AddMsgController}
	...
	public AddMsgController(AddMsgJFrame jFrame, MainJFrameController main) {
		...
		if (main.getKey() == null) {
			System.out.println(logPrefix +": error 0957: must be authenticated first");
	...
	private void submitBtnAction() {
		...
		try {
			if (!manager.registerToken(mainCtrl.getKey(), mainCtrl.getCiphertext())) {
				...
				System.out.println(AddMsgController.logPrefix + ": added message, '" + msg + "'");
				diagTitle = "Success";
			    diagText = "Message added";
			}
		} catch (InputMismatchException ex) {
			System.out.println(AddMsgController.logPrefix+": "+ex.getMessage());
