...
public class AddMsgJInternalController implements ActionListener {
	...
	/** {@link String} for identifying system log messages */
	...
	/**
	 * @param iFrame {@link AddMsgJFrame} sends events to this {@link AddMsgJInternalController}
	 * @param main {@link MainJFrameController} which provides the active session key
	 */
	public AddMsgJInternalController(AddMsgJInternalFrame iFrame, MainJFrameController main) {
		...
		if (main.getKey() == null) {
			System.out.println(logPrefix +": must be authenticated first");
	...
	/** marshals data from message fields and adds a {@link com.greentree.model.domain.Block} */
	private void submitBtnAction() {
		...
		try {
			if (!manager.registerToken(mainCtrl.getKey(), mainCtrl.getCiphertext())) {
				...
				System.out.println(AddMsgJInternalController.logPrefix + ": added message, '" + msg + "'");
				diagTitle = "Success";
			    diagText = "Message added";
			}
		} catch (InputMismatchException ex) {
			System.out.println(AddMsgJInternalController.logPrefix+": "+ex.getMessage());
	...
	private void keyBtnAction() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            System.out.println(logPrefix + ": opening " + file.getName());
        
	        // load a new frame with the log messages from the chosen key
			try (ObjectInputStream input = new ObjectInputStream(new FileInputStream(file))) {
				toKey = (RSAPublicKey) input.readObject();
				System.out.println(logPrefix + ": RSAPublicKey successfully parsed");
			} catch (Exception ex) {
				System.out.println(AddMsgJInternalController.logPrefix + ": " + ex.getMessage());
			}
		}
