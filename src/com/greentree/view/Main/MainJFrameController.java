...
public class MainJFrameController implements ActionListener {
	...
	/** This is prefixed to all log messages from this class. */
	private static String logPfx;
	
	/**
	 * Registers handlers for the given <code>{@link MainJFrameContainer}</code>
	 * 
	 * @param jFrame - <code>AuthJFrame</code> object that will receive handlers from this
	 */
	public MainJFrameController(MainJFrameContainer jFrame) {
		...
		if (!manager.registerService("TokenService")) {
			...
			System.out.println(logPfx + ": manager and listeners registered");
		}
	}
	
	/**
	 * constructs a new {@link MainJFrameController} with listeners attached to the menu items 
	 * of the given {@link MainJFrameDesktop}.
	 * 
	 * @param jFrame {@link MainJFrameDesktop} sending action events to this controller and 
	 *   receiving {@link JInternalFrame} objects from it in response
	 */
	public MainJFrameController(MainJFrameDesktop jFrame) {
		...
		if (!manager.registerService("TokenService")) {
			...
			System.out.println(MainJFrameController.logPfx + ": " + msg);
		    diag = new MessageDialog("Error 0927", msg);
		} else {
			System.out.println(MainJFrameController.logPfx + ": " + 
					           "manager and listeners registered");
		}
	}
	
	
	/**
	 * spawns a new JFrame appropriate to the {@link MainJFrameContainer} {@link JButton} pressed
	 */
	@Override
	public void actionPerformed(ActionEvent aev) {
		System.out.println(logPfx + ": actionPerformed(ActionEvent) " + aev.getActionCommand());
		...
		if (this.jFrameContainer instanceof MainJFrameContainer) {
			...
			if (aev.getSource().equals(jFrameContainer.getAuthJButton())) {
				System.out.println(logPfx + ": new AuthJFrame()");
			...
			if (aev.getSource().equals(jFrameContainer.getMsgJButton())) {
				System.out.println(logPfx + ": new AddMsgController(new AddMsgJFrame());");
			...
			if (aev.getSource().equals(jFrameContainer.getRegJButton())) {
				System.out.println(logPfx + ": new RegisterController(new RegisterJFrame());");
	        ...
	        if ("reg".equals(aev.getActionCommand())) {
				System.out.println(logPfx + ": new RegisterJInternalFrame();");
	        ...
	        if ("auth".equals(aev.getActionCommand())) {
				System.out.println(logPfx + ": new AuthJInternalFrame();");
	        ...
	        if ("new".equals(aev.getActionCommand())) {
	        	System.out.println(logPfx + ": new AddMsgJInternalController;");
			...
			System.out.println(logPfx + ": missing JFrame;");
		}
	}

	/**
	 * prompts the user for an {@link RSAPublicKey}, then reads all the available messages for the 
	 * associated {@link Token} using the <code>Token</code> registered in the {@link 
	...
	private void logButton_action(ActionEvent aev) {
		System.out.println(logPfx + ": " + aev.getActionCommand());
		...
		if (this.jFrameContainer instanceof MainJFrameContainer) {
			...
			System.out.println(logPfx + ": could not get state from JFileChooser");
			return;
		}		
		
		if (returnVal == JFileChooser.APPROVE_OPTION) {
            File file = fc.getSelectedFile();
            System.out.println(logPfx + ": opening " + file.getName() + ".");
            
            // load a new frame with the log messages from the chosen key
    		try (ObjectInputStream input = new ObjectInputStream(new FileInputStream(file))) {
    			RSAPublicKey key = (RSAPublicKey) input.readObject();
    			System.out.println(logPfx + ": RSAPublicKey successfully parsed");
            ...
            System.out.println(logPfx + ": Open command cancelled by user.");
        }
	}
	...
	/**
	 * @return {@link RSAPublicKey} used with ciphertext to auth {@link Token} objects
	 */
	...
	/**
	 * @param key {@link RSAPublicKey} used with ciphertext to authenticate a {@link Token}
	 */
